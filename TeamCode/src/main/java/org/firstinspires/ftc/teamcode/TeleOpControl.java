...
public class TeleOpControl extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        double lastIMU = robot.drive.getIMU();
        ...
        while (opModeIsActive()) {
            ...
            if (!fieldCentric) {
                ...
                if (lastIMU != imuValue) {
                    odometry.sync(imuValue, telemetry);
                    lastIMU = imuValue;
                }
                ...
                double profileY = MotionProfile.motion_profile(Odometry.MAX_ACCEL, Odometry.MAX_VELO, y, elapsed);
                double profileX = MotionProfile.motion_profile(Odometry.MAX_ACCEL, Odometry.MAX_VELO, x, elapsed);
                double profileH = MotionProfile.motion_profile(Odometry.MAX_ACCEL * 4, Odometry.MAX_VELO * 4, rx, elapsed);
...
////              double rotY = x * Math.sin(-botHeading) + y * Math.cos(-botHeading);
                ...
                rx = profileH; // just update the below later
